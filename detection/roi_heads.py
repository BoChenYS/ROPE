...
def heatmaps_to_keypoints(maps, rois):
    ...
    offset_x = rois[:, 0]
    offset_y = rois[:, 1]
    widths = scale*(rois[:, 2] - rois[:, 0])
    heights = scale*(rois[:, 3] - rois[:, 1])
    offset_x = offset_x - 0.5*(scale-1)*(rois[:, 2] - rois[:, 0])
    offset_y = offset_y - 0.5*(scale-1)*(rois[:, 3] - rois[:, 1])
...
def keypointrcnn_loss(keypoint_logits, proposals, gt_keypoints, keypoint_matched_idxs, sigma_t):
...
def distance_loss(dist_pred, dist_gt, pro_errs):
    # type: (Tensor, List[Tensor], List[Tensor], List[Tensor]) -> Tensor
    N, K, H, W = dist_pred.shape
    assert K+H+W == 3
    dist_gt_tensor = torch.cat(tuple(dist_gt), dim=0).squeeze()
    pro_errs_tensor = torch.cat(tuple(pro_errs), dim=0).squeeze()
    return 0.1*(((dist_pred.squeeze()-dist_gt_tensor)*(pro_errs_tensor<1))**2).mean()
...
def dist_pred_tolist(x):
    x_list = []
    bs = x.size(0)
    for i in range(bs):
        x_list.append(x[i].view(-1,1))
    return x_list
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 keypoint_head1=None,
                 keypoint_predictor1=None,
                 keypoint_head2=None,
                 keypoint_predictor2=None,
                 keypoint_head3=None,
                 keypoint_predictor3=None,
                 sigma1=1.5,
                 sigma2=3,
                 sigma3=8,
                 dist_head=None,
        ...
        self.keypoint_head1 = keypoint_head1
        self.keypoint_predictor1 = keypoint_predictor1
        self.keypoint_head2 = keypoint_head2
        self.keypoint_predictor2 = keypoint_predictor2
        self.keypoint_head3 = keypoint_head3
        self.keypoint_predictor3 = keypoint_predictor3
    ...
    def has_keypoint(self):
        if self.keypoint_roi_pool is None:
            return False
        if self.keypoint_head1 is None:
            return False
        if self.keypoint_predictor1 is None:
            return False
        return True
    ...
    def forward(self,
        ...
        # compile each branch
        if self.keypoint_roi_pool is not None and self.keypoint_head1 is not None \
                and self.keypoint_predictor1 is not None:
            ...
            roi_features = self.keypoint_roi_pool(features['0'], keypoint_proposals)

            keypoint_features1 = self.keypoint_head1(roi_features)
            keypoint_logits1 = self.keypoint_predictor1(keypoint_features1)

            if self.multi_precision:
                keypoint_features2 = self.keypoint_head2(roi_features)
                keypoint_logits2 = self.keypoint_predictor2(keypoint_features2)

                keypoint_features3 = self.keypoint_head3(roi_features)
                keypoint_logits3 = self.keypoint_predictor3(keypoint_features3)
            ...
            if self.training:
                ...
                rcnn_loss_keypoint1 = keypointrcnn_loss(
                    keypoint_logits1, keypoint_proposals,
                    gt_keypoints, pos_matched_idxs, sigma_t=self.sigma1)
                loss_keypoint = {
                    "loss_keypoint1": rcnn_loss_keypoint1
                }
                if self.multi_precision:
                    rcnn_loss_keypoint2 = keypointrcnn_loss(
                        keypoint_logits2, keypoint_proposals,
                        gt_keypoints, pos_matched_idxs, sigma_t=self.sigma2)
                    rcnn_loss_keypoint3 = keypointrcnn_loss(
                        keypoint_logits3, keypoint_proposals,
                        gt_keypoints, pos_matched_idxs, sigma_t=self.sigma3)
                    loss_keypoint.update({
                        "loss_keypoint2": rcnn_loss_keypoint2, "loss_keypoint3": rcnn_loss_keypoint3
                    })

            else:
                assert keypoint_logits1 is not None
                assert keypoint_proposals is not None

                keypoints_probs1, kp_scores1 = keypointrcnn_inference(keypoint_logits1, keypoint_proposals)
                for keypoint_prob1, kps1, r in zip(keypoints_probs1, kp_scores1, result):
                    r["keypoints1"] = keypoint_prob1
                    r["keypoints_scores1"] = kps1

                if self.multi_precision:
                    keypoints_probs2, kp_scores2 = keypointrcnn_inference(keypoint_logits2, keypoint_proposals)
                    for keypoint_prob2, kps2, r in zip(keypoints_probs2, kp_scores2, result):
                        r["keypoints2"] = keypoint_prob2
                        r["keypoints_scores2"] = kps2

                    keypoints_probs3, kp_scores3 = keypointrcnn_inference(keypoint_logits3, keypoint_proposals)
                    for keypoint_prob3, kps3, r in zip(keypoints_probs3, kp_scores3, result):
                        r["keypoints3"] = keypoint_prob3
                        r["keypoints_scores3"] = kps3
